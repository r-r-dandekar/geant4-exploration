...
{
    // You should like to check for volume overlaps
    // Volume overlaps are bad and should be avoided at all costs
    ...
    // The constructor for G4Box takes the HALF-width, height, depth so multiply by 0.5
    ...
    G4VPhysicalVolume *physLead = new G4PVPlacement(0, G4ThreeVector(0., 0., 5.0*cm), logicLead, "physLead", logicWorld, false, checkOverlaps);
    ...
    G4Box *solidDetector = new G4Box("solidDecetor", 0.5*detectorSize, 0.5*detectorSize, 0.5*detectorSize);
    logicDetector = new G4LogicalVolume(solidDetector, detMat, "logicDetector");     // logicDetector is declare in the header
    ...
    // Always return the mother volume
    return physWorld;
}
